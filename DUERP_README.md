...
- **Branche `staging`** → environnement de test, isolé :
  ...
  - `VITE_BYPASS_AUTH=true` peut être activé sur l'environnement **Preview** uniquement (jamais Production) pour contourner un blocage de connexion Clerk local lors des tests — à retirer une fois le blocage résolu, et à ne jamais laisser actif en dehors de Preview.
